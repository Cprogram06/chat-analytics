...
describe("split by token", () => {
    ...
    test.each(cases)('should split "$input" → $expected', ({ input, expected }) => {
        expect((0, Tokenizer_1.splitByToken)(input, DIGIT_MATCHER)).toStrictEqual(expected);
    });
});
...
it("exclude outside ' matching words", () => {
    const tokens = (0, Tokenizer_1.tokenize)("'hello'");
...
describe("URLs", () => {
    ...
    test.each(urlsToMatch)("should match %p", (url) => {
        const tokens = (0, Tokenizer_1.tokenize)(url);
        expect(tokens.length).toBe(1);
    ...
    test.each(urlsNotToMatch)("should not match %p", (url) => {
        const tokens = (0, Tokenizer_1.tokenize)(url);
        expect(tokens).not.toContain(expect.objectContaining({
            tag: "url",
        }));
    });
});
