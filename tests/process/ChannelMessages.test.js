...
const makePMessage = (i, authorId = 0) => ({ id: i, authorId, channelId: 1, timestamp: i });
const processFn = (m) => ({ dayIndex: 0, secondOfDay: 0, authorIndex: m.authorId });
it("should wait for EOF before processing last messages", () => {
    const fn = jest.fn((group) => group.map(processFn));
    const cm = new ChannelMessages_1.ChannelMessages();
...
describe("duplicate messages", () => {
    ...
    beforeEach(() => {
        ...
        cm = new ChannelMessages_1.ChannelMessages();
...
describe("complex sample", () => {
    ...
    it("iteration should return all elements", () => {
        expect(Array.from(cm.processedMessages())).toStrictEqual(expected.map((i) => ({ id: i, msg: processFn(makePMessage(i)) })));
    });
});
it("should crash if interval is extended into the past", () => {
    const interval = new ChannelMessages_1.MessagesInterval(makePMessage(69));
    expect(() => interval.addMessageAndExtend(makePMessage(68))).toThrow("MessagesInterval can only be extended to the future");
});
