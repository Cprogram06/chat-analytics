...
it("should convert from and to binary", () => {
    const day = Time_1.Day.fromKey("2020-6-7");
    const binary = day.toBinary();
    const back = Time_1.Day.fromBinary(binary);
...
describe("can be created from keys", () => {
    ...
    test.each(cases)("can be created from key: $key", ({ key, year, month, day }) => {
        const d = Time_1.Day.fromKey(key);
...
it("should advance one day", () => {
    const day = Time_1.Day.fromKey("2020-6-30");
...
it("should advance multiple days", () => {
    const day = Time_1.Day.fromKey("2020-6-29");
...
describe("operators", () => {
    const first = Time_1.Day.fromKey("2020-6-29");
    const days = new Array(60).fill(0).map((_, i) => first.nextDays(10 * i));
    test("operator eq", () => {
        for (let i = 0; i < days.length; i++) {
            for (let j = 0; j < days.length; j++) {
                expect(Time_1.Day.eq(days[i], days[j])).toBe(i === j);
            }
        }
    });
    test("operator lt", () => {
        for (let i = 0; i < days.length; i++) {
            for (let j = 0; j < days.length; j++) {
                expect(Time_1.Day.lt(days[i], days[j])).toBe(i < j);
            }
        }
    });
    test("operator gt", () => {
        for (let i = 0; i < days.length; i++) {
            for (let j = 0; j < days.length; j++) {
                expect(Time_1.Day.gt(days[i], days[j])).toBe(i > j);
            }
        }
    });
    test("operator min", () => {
        expect(Time_1.Day.min(days[0], days[1])).toBe(days[0]);
        expect(Time_1.Day.min(days[1], days[0])).toBe(days[0]);
    });
    test("operator max", () => {
        expect(Time_1.Day.max(days[0], days[1])).toBe(days[1]);
        expect(Time_1.Day.max(days[1], days[0])).toBe(days[1]);
    });
    test("operator clamp", () => {
        expect(Time_1.Day.clamp(days[5], days[10], days[20])).toBe(days[10]);
        expect(Time_1.Day.clamp(days[15], days[10], days[20])).toBe(days[15]);
        expect(Time_1.Day.clamp(days[25], days[10], days[20])).toBe(days[20]);
    });
});
test("genTimeKeys should generate keys correctly", () => {
    const { dateKeys, weekKeys, monthKeys, yearKeys, dateToMonthIndex, dateToWeekIndex, dateToYearIndex } = (0, Time_1.genTimeKeys)(Time_1.Day.fromKey("2020-12-25"), Time_1.Day.fromKey("2021-1-5"));
...
test("genTimeKeys should throw if start > end", () => {
    expect(() => (0, Time_1.genTimeKeys)(Time_1.Day.fromKey("2021-1-5"), Time_1.Day.fromKey("2020-12-25"))).toThrow();
});
