...
describe("full object", () => {
    ...
    `;
    beforeEach(() => {
        stream = new JSONStream_1.JSONStream();
        missingKeys = new Set(["string", "number", "boolean", "null", "array", "object"]);
        function register(key, expected) {
            stream.onObject(key, (got) => {
...
it("emit correct array objects", async () => {
    ...
    const nums = [], objs = [];
    stream.onArrayItem("arrVals", (got) => void nums.push(got));
    stream.onArrayItem("arrObjs", (got) => void objs.push(got));
    stream.push(`
        {
            "a": "b",
            "arrVals": [1, 2, 3],
...
describe("escaping", () => test.each([`\\\\`, `\\n`, `\\"`, `\\u00f8`])("escape with %p", async (esc) => {
    new JSONStream_1.JSONStream().push(`{ "a${esc}b": "c" }`);
    new JSONStream_1.JSONStream().push(`{ "a": "b${esc}c" }`);
    new JSONStream_1.JSONStream().push(`{ "a": { "c": "d${esc}e" } }`);
}));
it("string with brackets", () => void new JSONStream_1.JSONStream().push(`{ "a": "c{}{[][}{[!}{}[<}{]\\n][]]]}[>[][" }`));
it("string with unicode characters", () => void new JSONStream_1.JSONStream().push(`{ "a": "◄💩💩💩💩►", "b": "◄💩💩💩💩►", "c": "normal☰☰☰" }`));
async function expectCrash(input) {
    const stream = new JSONStream_1.JSONStream();
