...
describe("writing", () => {
    ...
    test.each(cases)("$name: offset=$offset bits=$bits", ({ offset, bits, input, previous, expected }) => {
        ...
        buf[0] = previous[0];
        buf[1] = previous[1];
        const s = new BitStream_1.BitStream(buf.buffer);
        s.offset = offset;
...
it("should get bits correctly after lots of sets", () => {
    const s = new BitStream_1.BitStream();
    const bits = [], values = [];
    for (let k = 0; k < 100; k++) {
        for (let i = 1; i <= 32; i++) {
...
test.each([7, 8, 9, 10, 11, 15, 16, 17, 20, 24, 31, 32, 48])("varint %s bits should write and read correctly", (bits) => {
    const cases = [
        0,
        100,
        200,
    ...
    for (const value of cases) {
        let s = new BitStream_1.BitStream();
        s.offset = 0;
...
it("should crash if created with an invalid buffer", () => {
    expect(() => new BitStream_1.BitStream(new Uint8Array())).toThrow("buffer must not be a view");
    expect(() => new BitStream_1.BitStream(new ArrayBuffer(7))).toThrow("buffer must be aligned to 32bits"); // must be a multiple of 4 bytes
});
