...
const fn = (database, filters, common, args) => {
    ...
    const channelConversations = new Array(database.channels.length).fill(0);
    const authorConversations = new Array(database.authors.length).fill(0);
    // first, find the MAX_AUTHORS authors with the most messages
    const authorsCounts = [...authorConversations];
    ...
    (0, Helpers_1.filterMessages)(processMessage, database, filters, { channels: true, authors: false, time: true });
    // generate the nodes
    const nodes = [];
    for (let i = 0; i < N; i++) {
        for (let j = i + 1; j < N; j++) {
