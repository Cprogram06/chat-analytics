...
class BitStream {
    constructor(buffer) {
        if (buffer) {
            if (ArrayBuffer.isView(buffer))
                throw new Error("buffer must not be a view");
            if (buffer.byteLength % 4 !== 0)
                throw new Error("buffer must be aligned to 32bits");
        }
    ...
    setBits(bits, value) {
        ...
        if ((offset + bits) / 8 > this.buffer.byteLength - 4)
            this.grow();
    ...
    readVarInt(maxBits = 32) {
        if (maxBits < 10)
            return this.getBits(maxBits);
        if (maxBits > 30) {
            // big int
            let value = 0n;
            let byte = 0n;
