...
exports.matchAttachmentType = exports.isGroupWelcome = exports.removeBadChars = exports.extractChatName = void 0;
...
const extractChatName = (filename) => {
    let name;
    for (const template of FilenamePatterns) {
        // expected: <pattern><chat name>.txt
        if ((0, Text_1.matchFormat)(filename).startsWith((0, Text_1.matchFormat)(template)) &&
...
exports.extractChatName = extractChatName;
// some exports include those chars for some reason, remove them
const removeBadChars = (str) => str.replace(/[\u202a\u200e\u202c\xa0]/g, "");
exports.removeBadChars = removeBadChars;
...
const isGroupWelcome = (content) => WelcomePatterns.some((pattern) => content.toLocaleLowerCase().includes(pattern.toLocaleLowerCase()));
exports.isGroupWelcome = isGroupWelcome;
const Patterns = {
...
// reduce Patterns
const normalize = (x) => x.toLocaleLowerCase().trim();
const reducePatterns = (key) => {
    const res = [];
    for (const lang in Patterns) {
        const list = Patterns[lang][key];
        if (list)
            res.push(...list.map(normalize));
    }
    return res;
};
const PatternsFlat = {
...
const PatternsType = {
    ...
    media: Attachments_1.AttachmentType.Other,
    // specific
    image: Attachments_1.AttachmentType.Image,
    video: Attachments_1.AttachmentType.Video,
    audio: Attachments_1.AttachmentType.Audio,
    document: Attachments_1.AttachmentType.Document,
    GIF: Attachments_1.AttachmentType.ImageAnimated,
    sticker: Attachments_1.AttachmentType.Sticker,
};
const matchAttachmentType = (input) => {
    const _input = normalize(input);
    let key;
    for (key in PatternsType) {
        for (const pattern of PatternsFlat[key]) {
            if (_input.endsWith(pattern))
                return PatternsType[key];
        }
    }
    return undefined;
};
exports.matchAttachmentType = matchAttachmentType;
